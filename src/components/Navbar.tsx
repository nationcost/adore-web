...
const Navbar: React.FC = () => {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `text-sm font-medium transition-all duration-300 hover:text-white px-4 py-2 rounded-full ${isActive ? 'text-white bg-white/10' : 'text-white/70 hover:bg-white/5'
    }`;

  // Custom styles for the mobile menu items
  const mobileLinkClass = ({ isActive }: { isActive: boolean }) =>
    `block w-full text-center py-6 text-xl font-medium transition-all duration-200 ${isActive ? 'text-white' : 'text-gray-400 active:text-white'
    }`;
